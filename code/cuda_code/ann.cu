...
ann_t * create_ann(double alpha, unsigned minibatch_size, unsigned number_of_layers, unsigned* nneurons_per_layer)
{
    ann_t * nn;
    layer_t * layer;
    CHECK_CUDA_ERROR(cudaMallocManaged((void**)&nn, sizeof(ann_t)));
    CHECK_CUDA_ERROR(cudaMallocManaged((void**)&nn->layers, number_of_layers * sizeof(layer_t *)));
    nn->number_of_layers = number_of_layers;
    nn->alpha = alpha;
    nn->minibatch_size = minibatch_size;
...
{
    layer_t * layer;
    CHECK_CUDA_ERROR(cudaMallocManaged((void**)&layer, sizeof(layer_t)));
...
void forward(ann_t *nn, double (*activation_function)(double))
{
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        // launch kernel for matrix_dot
        dim3 blockDim(16, 16);
        dim3 gridDim((nn->layers[l]->weights->columns + blockDim.x - 1) / blockDim.x, (nn->layers[l-1]->activations->rows + blockDim.y - 1) / blockDim.y);
        matrix_dot<<<gridDim, blockDim>>>(nn->layers[l]->weights, nn->layers[l-1]->activations, z1);
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());

        // launch kernel for matrix_dot
        dim3 blockDim2(16, 16);
        dim3 gridDim2((nn->layers[l]->biases->columns + blockDim2.x - 1) / blockDim2.x, (one->rows + blockDim2.y - 1) / blockDim2.y);
        matrix_dot<<<gridDim2, blockDim2>>>(nn->layers[l]->biases, one, z2);
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
...
void backward(ann_t *nn, matrix_t *y, double (*derivative_actfunct)(double))
{
    ...
    for (int l = L; l > 1; l--)
    {
        ...
        // launch kernel for matrix_dot
        dim3 blockDim(16, 16);
        dim3 gridDim((tw->columns + blockDim.x - 1) / blockDim.x, (nn->layers[l]->delta->rows + blockDim.y - 1) / blockDim.y);
        matrix_dot<<<gridDim, blockDim>>>(tw, nn->layers[l]->delta, delta_tmp);
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
    ...
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        // launch kernel for matrix_dot
        dim3 blockDim(16, 16);
        dim3 gridDim((nn->layers[l]->delta->columns + blockDim.x - 1) / blockDim.x, (ta->rows + blockDim.y - 1) / blockDim.y);
        matrix_dot<<<gridDim, blockDim>>>(nn->layers[l]->delta, ta, w1);
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
        ...
        // launch kernel for matrix_dot
        dim3 blockDim2(16, 16);
        dim3 gridDim2((nn->layers[l]->delta->columns + blockDim2.x - 1) / blockDim2.x, (one->rows + blockDim2.y - 1) / blockDim2.y);
        matrix_dot<<<gridDim2, blockDim2>>>(nn->layers[l]->delta, one, b1);
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
